...
sub printUsage {
    print
"Usage: $0 --help|--bs=1M --count=1000 --vector=<readSpeed|writeSpeed> --mode=serial --dir=/data/1/,/data/2,/data/3,/data/4,/data/5 \
...
sub runWriteBM {
    ...
    printf WLOG "$name sized $size KB written in $duration seconds, Thruput %.2f KB/s\n", $thruput;
    close(WLOG);
    return (0);
}
...
sub runReadBM {
    ...
    open( WLOG, ">> $logfile" ) or die " Unable to write to $logfile
        : $! \n ";
    my $thruput = $size / $duration;
    printf WLOG "$readfile sized $size KB read in $duration seconds, Thruput %.2f KB/s\n", $thruput;
    close(WLOG);
    return (0);
}
...
if (! $pid) {
   ...
   if ( ! -f $collectl ) {
	print "$collectl is not present on the system...will continue running without collecting OS stats: !$\n";
   }
   else {
       my $colopt = "-sdmnc -i 5 -P -f /var/tmp/$outdir";
      exec("/usr/bin/collectl -sdDmnc -i 5 -P -f /var/tmp/$outdir") or die "Could not exec: !$\n";
   }
}

if ( $vector =~ m/writeSpeed/i ) {
    chomp $dir;
    my @disks = split( ',', $dir );
    my $numdirs = @disks;
