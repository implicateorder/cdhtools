...
hosts = [ ]
cm_host = config.get('master', 'cmnode')
cm_admin = config.get('master', 'cmadmin')
cm_passwd = config.get('master', 'cmpasswd')
master_node = config.get('master', 'nodes')
worker_nodes = config.get('worker', 'nodes')
cluster_name = config.get('cluster', 'name')
#wnodes = worker_nodes.split(" ")
hosts.append(worker_nodes)
hosts.append(master_node)

#print(hosts)

api = ApiResource(cm_host, username=cm_admin, password=cm_passwd)

# Distribute the CDH parcel

repo_ver = config.get('cluster', 'repoversion')
parcel_name = config.get('cluster', 'parcel')

parcel_repo = 'http://archive.cloudera.com/cdh5/parcels/'+ repo_ver + '/'
...
#parcel = cluster.get_parcel("CDH", parcel_name)
parcel = cluster.get_parcel("CDH", parcel_name)
...
name_node = config.get('hdfs', 'nn')
secondary_name_node = config.get('hdfs', 'snn')
data_node = config.get('hdfs', 'dn')
data_nodes = data_node.split(" ")
hdfs_mpts = config.get('hdfs', 'mountpoints')
nn_mount = config.get('hdfs', 'nnmount')
snn_mount = config.get('hdfs', 'snnmount')
...
nn_config = {
   'dfs_name_dir_list': nn_mount,
   'dfs_namenode_handler_count': 30,
}
snn_config = {
   'fs_checkpoint_dir_list': snn_mount,
}
dn_config = {
   'dfs_data_dir_list': hdfs_mpts ,
...
hdfs.create_role("NAMENODE-MASTER", "NAMENODE", name_node)
hdfs.create_role("SECONDARYNAMENODE-MASTER", "SECONDARYNAMENODE", secondary_name_node)
count = 1
for node in data_nodes:
    dnlabel = 'DATANODE' + count
    hdfs.create_role(dnlabel, "DATANODE", node)
    count = count + 1
