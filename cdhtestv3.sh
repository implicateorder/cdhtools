...
mapreduce_jar=/opt/cloudera/parcels/CDH/lib/hadoop-0.20-mapreduce/hadoop-examples.jar
...
while getopts p:x:m:M:r:R:d:s:i:o:h switch
do
    case $switch in 
	p) phase=$OPTARG;;
...
case $shuffle in
	...
	*) echo "Uknown shuffle type option - only accept <regular|uda>\n" && exit 1;;
esac
